...
try {
    ...
    if ($currentUser['role'] !== 'manager') {
        $_SESSION['flash_error'] = 'Access denied. Manager privileges required.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_project'])) {
    ...
    if (!empty($members)) {
        // Only allow employees to be assigned as members (exclude admins)
        $placeholders = str_repeat('?,', count($members) - 1) . '?';
        $stmtCheckMembers = $db->prepare("SELECT COUNT(*) FROM users WHERE user_id IN ($placeholders) AND role = 'employee'");
        $stmtCheckMembers->execute($members);
        if ($stmtCheckMembers->fetchColumn() != count($members)) {
            $_SESSION['flash_error'] = 'One or more selected members are invalid. Only employees can be assigned to projects.';
            header('Location: manage_projects.php');
            exit;
        }
    }
...
// Only get employees for member selection (exclude admins)
$usersStmt = $db->query('SELECT user_id, full_name, role FROM users WHERE role = "employee" ORDER BY full_name ASC');
...
<head>
  ...
  <link rel="stylesheet" href="assets/css/manage_projects.css">
</head>
<body>
  <div class="d-flex">
    <?php $active='projects'; include 'assets/sidebar/sidebar.php'; ?>
  ...
  <script src="assets/sidebar/sidebar.js" defer></script>
  <script src="assets/js/manage_projects.js" defer></script>
  <script>
